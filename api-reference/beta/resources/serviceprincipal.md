...
|description| String | Free text field to provide an internal end-user facing description of the service principal. The maximum allowed size is 1024 characters.|
...
|preferredTokenSigningKeyThumbprint|String| Specifies the thumbprint of the custom signing key in [keyCredentials](keycredential.md) used to sign the token. You need to use custom signing key if the service principal was created from an [applicationTemplates](applicationtemplate.md). |
